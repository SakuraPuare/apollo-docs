...
- **向后兼容**：通过 OldRoutingAdapter 兼容 9.0 之前版本的 RoutingRequest 接口
...
| `SpeedCommandProcessor` | `SpeedCommand` | `/apollo/external_command/speed` | 更改目标速度（园区） |
| `ChassisCommandProcessor` | `ChassisCommand` | `/apollo/external_command/chassis` | 自定义底盘命令（园区） |
...
- 继承自 `cyber::Component<>`
- 职责：兼容 Apollo 9.0 之前版本的 `RoutingRequest` 接口
...
| `/apollo/canbus/chassis_control` | `ChassisCommand` | 底盘命令发送给 Canbus |
