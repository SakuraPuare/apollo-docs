...
`adapters` 子模块通过 `adapter_flags` 定义了 Apollo 系统中 80+ 个 Topic 名称，是模块间基于 Cyber RT 消息通信的基础。每个 Topic 对应一个 `DEFINE_string` 类型的 gflag，上层模块通过引用这些 flag 来订阅或发布消息，避免硬编码 Topic 字符串。
...
存放车辆物理参数配置文件，包括车辆尺寸、轮距、最大转向角等。这些配置以 Protobuf 文本格式或 JSON 存储，在系统启动时加载，供规划、控制等模块使用。
...
| `LinearInterpolation` | 线性插值，用于轨迹点补全 |
| `IntegralSolver` | 数值积分 |
| `CartesianFrenetConverter` | 笛卡尔坐标与 Frenet 坐标互转 |

#### monitor_log — 日志与监控

提供统一的日志记录接口，支持将日志消息发送至 Dreamview 的监控面板。日志级别包括 INFO、WARN、ERROR、FATAL，与 Apollo 的 `MonitorMessage` proto 对应。

```cpp
// 使用示例
AINFO << "Planning module initialized";
AERROR << "Failed to get localization data";
```

#### proto — 公共序列化定义

定义 `common` 模块自身使用的 Protobuf 消息，包括：
...
这些 proto 与 `common_msgs` 中的消息互补——`common/proto` 侧重模块内部使用，`common_msgs` 侧重跨模块通信。
...
- **Topic 名称**：通过 gflags 在 `adapter_flags` 中定义，可通过命令行参数或 flagfile 覆盖
- **车辆参数**：`configs/data/` 下的配置文件，Protobuf 文本格式
...
该模块包含 28 个消息子目录，覆盖音频、底盘、控制、感知、规划、定位、地图等全部子系统。
...
| `ErrorCode` | 统一错误码枚举 |
| `PointENU` | 东-北-天坐标系下的三维点 |
| `SLPoint` | Frenet 坐标系下的点（纵向 s，横向 l） |
| `PathPoint` | 路径点，包含位置、航向、曲率 |
| `SpeedPoint` | 速度-时间点，包含 s、t、v、a、jerk |
| `Quaternion` | 四元数，用于姿态表示 |
...
message ControlCommand {
  optional double throttle = 1;   // 油门开度 [0, 100]
  optional double brake = 2;      // 制动开度 [0, 100]
  optional double steering_target = 3;  // 目标转向角
  // ...
}
```
