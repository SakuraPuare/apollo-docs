...
Apollo 的包管理系统要求每个库都以共享库形式存在，以支持二进制包的分发和动态加载。`apollo_cc_library` 宏内部会同时生成一个 `cc_binary`（`linkshared=True`）产出 `.so`，以及一个 `cc_library`（`alwayslink=True`）用于源码编译时的静态链接。
...
`cpplint()` 遍历当前 BUILD 文件中所有规则的 `srcs` 和 `hdrs`，为每个包含 C++ 源文件的规则生成一个 `py_test` 目标（名为 `<rule_name>_cpplint`），运行 Google cpplint 工具检查代码风格。配置文件从项目根目录的 `CPPLINT.cfg` 和当前目录的 `CPPLINT.cfg` 中读取。
