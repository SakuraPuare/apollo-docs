...
内部实现逻辑：
1. 将 `srcs` 和 `hdrs` 合并，通过 `CC_BINARY` 生成 `lib<name>.so`（`linkshared=True, linkstatic=True`）
...
内部实现：
1. 调用 `cyber_plugin_description` 注册插件描述
2. 通过 `CC_LIBRARY` 创建 `<name>_lib` 内部库（`alwayslink=True`）
3. 通过 `CC_BINARY` 以 `linkshared=True, linkstatic=True` 生成最终 `.so`
...
| 参数 | 类型 | 说明 |
|------|------|------|
| `hdrs` | `list[label]` | 头文件，内部自动判断是否包含 `Q_OBJECT` 并进行 MOC 处理 |
...
4. `<name>_hdrs` — `cc_proto_library`，导出头文件
...
| `if_profiler()` | 性能分析器开关（不接受参数，直接返回编译宏） | `-DENABLE_PROFILER=1` / `0` |
